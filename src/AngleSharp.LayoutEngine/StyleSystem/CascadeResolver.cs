...
public class CascadeResolver
{
        ...
        IElement element)
    {
        ...
        // Get the browsing context from the element's document
        var context = element.OwnerDocument?.Context;
        if (context == null)
            throw new InvalidOperationException("Element must have a browsing context");


        // Create a new style declaration to hold resolved properties
        var resolvedStyle = new CssStyleDeclaration(context);
