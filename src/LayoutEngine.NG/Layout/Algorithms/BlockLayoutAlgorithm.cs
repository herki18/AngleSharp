...
public class BlockLayoutAlgorithm : LayoutAlgorithm
{
    ...
    public BlockLayoutAlgorithm(BlockNode node, ConstraintSpace constraintSpace)
        : base(node, constraintSpace)
    {
        ...
        _style = node.GetStyle() ?? throw new InvalidOperationException("Node must have computed style");
    }
    ...
    private void LayoutChildren()
    {
        ...
        while (child != null)
        {
            ...
            else if (child.IsText)
            {
                ...
                var textFragment = LayoutTextChild(child, childAvailableInlineSize, currentBlockOffset);
    ...
    private PhysicalFragment? LayoutTextChild(
        LayoutInputNode text,
        float availableInlineSize,
        float blockOffset)
    {
        // Very simplified text layout
        var textContent = text.GetTextContent();
        ...
        return PhysicalFragment.CreateBuilder()
            ...
            .SetOffset(new PhysicalOffset(
                ...
                blockOffset))
            .SetLayoutObject(text.GetLayoutBox())
