...
namespace LayoutEngine.Platform
{
    ...
    public static class SimplifiedServiceExtensions
    {
        ...
        public static IServiceCollection AddSimplifiedPlatformServices(this IServiceCollection services)
        {
            ...
            // services.TryAddSingleton<IIdleTaskScheduler, SimplifiedFrameScheduler>();

            // Register time provider
            // services.AddSingleton<ITimeProvider, SystemTimeProvider>();

            // Register frame timing strategy
            // services.AddSingleton<IFrameTimingStrategy, SynchronousFrameTimingStrategy>();

            // Register simplified threading coordinator
            // services.AddSingleton<SimplifiedThreadingCoordinator>();
            // services.AddSingleton<IThreadingCoordinator>(sp => sp.GetRequiredService<SimplifiedThreadingCoordinator>());
            //
            // // Register simplified schedulers
            // services.AddSingleton<SimplifiedFrameScheduler>();
            // services.AddSingleton<IFrameScheduler>(sp => sp.GetRequiredService<SimplifiedFrameScheduler>());
            //
            // services.AddSingleton<SimplifiedUpdateScheduler>();
            // services.AddSingleton<IUpdateScheduler>(sp => sp.GetRequiredService<SimplifiedUpdateScheduler>());
